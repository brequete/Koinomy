...
# Design System — Koinomy

**Sync rule.** This document is the single source of truth for the Koinomy interface: color palette, typography, spacing, shapes, and component guidance. Any PR that changes a visual token, a component's treatment, or the implementation mapping in §Components MUST update this document in the same PR. This system was faithfully implemented in v1 and carried over without redesign (ADR-0001); its stability is the point — do not redesign it, extend it through the iteration guide at the end.
...
**Adoption in Koinomy.** Koinomy is a private, self-hosted personal financial application, not a marketing site — the dark-canvas orientation is the default for its product surfaces (dashboard, ledger, accounts, budgets), and the light theme is retained for transactional dialogs and forms exactly as specified below. Marketing-only compositions (hero bands, promo cards, footer) remain documented as the reference vocabulary for any future public-facing surface. Implementation is shadcn/ui + Radix primitives over Tailwind 4 (ADR-0002); v1 hand-rolled components are not carried over — the mapping in §Components names the shadcn/ui equivalent for every component.
...
### Implementation Mapping (shadcn/ui + Radix over Tailwind 4)

Koinomy builds this system with **shadcn/ui components over Radix primitives, styled with Tailwind 4** (ADR-0002). v1 hand-rolled components are not carried over (ADR-0001); the visual contract above is unchanged, only the implementation substrate differs.

**Token wiring.** The frontmatter tokens map 1:1 into the Tailwind 4 theme (`@theme` CSS variables in `client/`): `colors.*` → color tokens, `rounded.*` → radius tokens, `spacing.*` → spacing tokens, `typography.*` → font-size/weight/line-height utilities composed per role. Canvas modes use the class-based dark strategy: dark-canvas surfaces are the app default; light (transactional) surfaces opt in per component, exactly as the multi-theme rule above describes.

**Component mapping.**

| Design-system component | Koinomy implementation |
|-------------------------|-------------------|
| `button-primary`, `button-primary-active`, `button-primary-disabled` | shadcn/ui `Button` — default variant restyled to the yellow/black contract (pressed and disabled states included) |
| `button-primary-pill` | `Button` with pill radius variant |
| `button-secondary-on-dark` / `button-secondary-on-light` | `Button` secondary variant, surface tone per canvas mode |
| `button-tertiary-text`, `text-link` | `Button` link variant |
| `button-trading-up` / `button-trading-down` | `Button` semantic variants — price-direction actions only |
| `button-subscribe` | `Button` compact size |
| `top-nav-dark` / `top-nav-light` | App shell layout + `NavigationMenu` |
| `markets-table-card`, `markets-row`, `price-up-cell`, `price-down-cell`, `trader-row`, `conversion-cell` | `Table` with per-cell typography and direction coloring |
| `markets-table-card` tab row | `Tabs` |
| `faq-row` | `Accordion` |
| `search-input-on-dark`, `text-input-on-light` | `Input` |
| `cookie-consent-card` and all modal dialogs | `AlertDialog` / `Dialog` |
| `trust-badge`, `stat-callout-card`, `steps-card`, `buy-crypto-amount-card`, `price-chart-card`, `qr-promo-card`, `feature-photo-card`, `cta-band-dark` | `Card` compositions with the token treatments above |
| `hero-band-dark`, `funds-safu-band`, `arena-hero-gradient`, `footer-light` | Layout sections (composed, not shadcn components) — reference vocabulary for public-facing surfaces |

**Forms.** All forms are react-hook-form + zod resolvers (`docs/ARCHITECTURE.md` §9); input components carry the focus-ring treatment (`{colors.info-ring}` at 50% alpha) from the elevation table.
...
- Form validation states beyond `{component.text-input-on-light}` defaults are not extracted — error / success input variants would need a sign-up or order-confirmation flow to confirm. **Koinomy note:** validation states are defined when the first form re-ports (react-hook-form + zod, `docs/TESTS.md` §8); per the Do's and Don'ts they must not reuse `{colors.trading-up}` / `{colors.trading-down}`, which stay reserved for price direction.
